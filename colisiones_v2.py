    ...
    if (measurement_mode or momentum_mode) and event == cv2.EVENT_LBUTTONDOWN:
        ...
        if closest:
            if len(selection_points) == 1 and closest['ball_id'] != selection_points[0]['ball_id']:
                ...
                print(f"Cambiado al balón {closest['ball_id']}")
            else:
                selection_points.append(closest)
                print(f"Punto {len(selection_points)} seleccionado para el balón {closest['ball_id']}")
                ...
                if v is not None:
                    ...
                    if momentum_mode:
                        if len(velocity_results) == 4:
                            ...
                            mag_i = np.linalg.norm(p_inicial)
                            mag_f = np.linalg.norm(p_final)
                            conservacion = (mag_f / mag_i * 100) if mag_i > 0 else 0

                            print("\n--- Análisis de Conservación de Momentum ---")
                            print(f"Vector de velocidad inicial total (P/m): {p_inicial}")
                            print(f"Vector de velocidad final total   (P/m): {p_final}")
                            print(f"Momentum conservado: {conservacion:.1f}%")
                            print("--------------------------------------------\n")
                    else:
                        if len(velocity_results) >= 2:
                            ...
                            if v1['ball_id'] == v2['ball_id']:
                                ...
                                print(f"Coeficiente de Restitución (Estimado): {e_val:.3f}")
                selection_points = []
...
def plot_trajectory_data(history, start_idx, end_idx, ball_id):
    ...
    plt.suptitle(f'Trayectoria — Balón {ball_id}', fontsize=12)

    plt.subplot(211)
    plt.scatter(t, x, s=10)
    plt.plot(t, sx * t + ix, 'r', label=f'Vx={sx:.2f} cm/s  (R²={rx**2:.2f})')
    plt.ylabel('Posición X (cm)')
    plt.legend(); plt.grid()

    plt.subplot(212)
    plt.scatter(t, y, s=10)
    plt.plot(t, sy * t + iy, 'g', label=f'Vy={sy:.2f} cm/s  (R²={ry**2:.2f})')
    plt.xlabel('Tiempo (s)')
    plt.ylabel('Posición Y (cm)')
    plt.legend(); plt.grid()

    plt.tight_layout()
    plt.savefig('vectores_colision.pdf', bbox_inches='tight')
    plt.show()
...
def run_collisions(model_path):
    ...
    cv2.namedWindow("Analizador de Colisiones")
    cv2.setMouseCallback("Analizador de Colisiones", onMouse)
    ...
    while True:
        ...
        if not calib.is_calibrated():
            ...
            estado = "MODO MOMENTUM"
        elif measurement_mode:
            estado = "MODO MEDICIÓN"
        else:
            estado = "RASTREANDO"

        cv2.putText(disp, f"E:{estado} | S:Iniciar P:Pausar M:Medir K:Momentum C:Limpiar R:Reiniciar Q:Salir",
        ...
        if calib.is_calibrated() and not tracking_active:
            cv2.putText(disp, "Verifique la cuadricula. Si esta distorsionada, presione 'R' para recalibrar.",
                        (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

        cv2.imshow("Analizador de Colisiones", disp)
        ...
        if key == ord('q'): break
            ...
            velocity_results = []
            estado_modo = 'ACTIVADO' if momentum_mode else 'DESACTIVADO'
            print(f"Modo Momentum {estado_modo}. Seleccione 4 segmentos en orden: B1_inicial, B2_inicial, B1_final, B2_final.")
        elif key == ord('m'):
            ...
            velocity_results = []
            selection_points = []
            print("Mediciones borradas.")
        elif key == ord('r'):
            point_history_1.clear(); point_history_2.clear()
            last_pos_px_1 = None; last_pos_px_2 = None
            calib.reset(); velocity_results = []
