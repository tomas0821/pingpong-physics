...
def plot_pendulum_data(history, avg_period):
    ...
    try:
        ...
        plt.plot(t_smooth, np.degrees(theta_fit), 'g-', label='Ajuste Amortiguado')
        A, beta, omega, phi = popt
        T_fit = 2 * np.pi / omega
        print(f"\n--- Ajuste de Oscilación Amortiguada ---")
        print(f"  Amplitud inicial A₀ : {np.degrees(A):.2f}°")
        print(f"  Coef. amortiguamiento β : {beta:.4f} s⁻¹")
        print(f"  Frecuencia angular ω : {omega:.4f} rad/s")
        print(f"  Período T : {T_fit:.3f} s")
        print(f"  Fase inicial φ : {np.degrees(phi):.2f}°")
        print(f"----------------------------------------\n")
    except Exception:
        pass

    plt.plot(t, np.degrees(theta), 'b.', label='Datos', markersize=2)
    plt.title('Péndulo: Ángulo vs Tiempo')
    plt.xlabel('Tiempo (s)')
    plt.ylabel('Ángulo θ (grados)')
    plt.grid(); plt.legend()
    plt.savefig('pendulo_ajuste.pdf', bbox_inches='tight')
    print("Gráfica guardada en pendulo_ajuste.pdf")
    plt.show()
...
def run_pendulum(model_path):
    ...
    while True:
        ...
        if avg_T > 0:
            cv2.putText(disp, f"Período T={avg_T:.3f} s", (10, 60),
        ...
        cv2.putText(disp, f"E:{estado} | S:Iniciar G:Grafica R:Reiniciar Q:Salir",
        ...
        if key == ord('q'): break
            ...
            pivot_point_px = None; pivot_point_cm = None
            calib.reset(); pendulum_history.clear()
